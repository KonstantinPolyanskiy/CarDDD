...
public class CarCreatedHandler(ILogger<CarCreatedHandler> log, IIntegrationPublisher publisher, IApplicationUserStorage userStorage) : IDomainEventHandler<CarCreated>
{
    public async Task HandleAsync(CarCreated @event, CancellationToken ct = default)
    {
        ...
        if (!@event.HasPhoto)
        {
            var manager = await userStorage.ReadAsync(@event.ManagerId.Value);
            if (manager == null)
            {
